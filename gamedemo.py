...
class BallInMaze(DirectObject):
  ...
  def __init__(self):
    ...
    self.world.setGravity(Vec3(0, 0, -8.81)) #lower gravity so ball slides down more slowly
    self.world.setDebugNode(self.debugNP.node())

    endShape = BulletBoxShape(Point3(1, 1, 1))  # create a box shape for end marker
    endNP = render.attachNewNode(BulletGhostNode('End')) #creates a ghost node and attaches to render node
    endNP.node().addShape(endShape)
    endNP.setPos(5,4, -1)  # sets postion at checkmark
    self.world.attachGhost(endNP.node())
    self.end = endNP
    ...
    self.ballNP.node().setMass(1.2)  #increase mass from 1.0 to 1.2
    self.ballNP.setPos(4, -4, 1)
    self.ballNP.node().setFriction(0.8)  # Increase friction from 0.5 to 0.8
  ...
  def removeHole(self, holeNP):
    self.world.removeGhost(holeNP.node())
    self.holes.remove(holeNP)
    holeNP.removeNode()  
  ...
  def updateGame(self, task):
    ...
    # Check if ball has reached the end of the platform
    if self.end.node().getNumOverlappingNodes() > 0:
        if (self.ballNP.node() in self.end.node().getOverlappingNodes()):
            self.winGame()
  ...
  def looseGame(self, holeNP):
    print("Sorry, you have lost the game - the ball is in the hole")
